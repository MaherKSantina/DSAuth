...
final class DSAuthTests: WMSTestCase {
    func testLogin_ShouldLoginProperly() throws {
        ...
        let login1 = try User_LoginRow.find(email: "user1@gmail.com", password: "asdfgh", organizationID: 1, on: conn).wait()
        XCTAssertNotNil(login1)

        let login2 = try User_LoginRow.find(email: "user1@gmail.com", password: "asdfgh", organizationID: nil, on: conn).wait()
        XCTAssertNil(login2)

        let login3 = try User_LoginRow.find(email: "user2@gmail.com", password: "123456", organizationID: nil, on: conn).wait()
        XCTAssertNotNil(login3)
    }

    func testRegister_NewUser_NewLogin_OrganizationDoesntExist_ShouldShowError() throws {
        do {
            let _ = try DSAuthMain.register(user: UserRow.Register(email: "maher.santina90@gmail.com", password: "123456", organizationID: 1), on: conn).wait()
            XCTFail()
        }
        catch {
    ...
    func testRegister_NewUser_NewLogin_OrganizationExists_ShouldRegisterProperly() throws {
        let org = try OrganizationRow(id: nil, name: "Org 1").save(on: conn).wait()
        let _ = try DSAuthMain.register(user: UserRow.Register(email: "maher.santina90@gmail.com", password: "123456", organizationID: try org.requireID()), on: conn).wait()
    }
    ...
    func testConvertAccessDto_ShouldConvertProperly() throws {
        ...
        let accessDto = try DSAuthMain.login(user: LoginRow.Post(email: "maher.santina90@gmail.com", password: "123123", organizationID: try org.requireID()), on: conn).wait()
